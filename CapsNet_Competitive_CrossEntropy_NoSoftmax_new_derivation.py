...
# n_epochs = 10
checkpoint_path = checkpoint_path[0:len(checkpoint_path)-1]+"_CCE_NSM/" # no softmax
...
# caps2_output_round_2 = weighted_sum_round_2 # no squashing
...
def ECC_loss(outputLayer):
  ...
  output_norm = safe_norm(outputLayer, axis=-1)

  yy = output_norm / tf.reduce_sum(output_norm,axis=1,keep_dims=True)
  ...
  outputLayer_masked = tf.multiply(output_norm , mask)

  # denom = tf.reduce_sum(outputLayer_exp_masked ,axis=1,keep_dims = True)
  denom = tf.reduce_sum(outputLayer_masked, axis=1,keep_dims=True)

  # tau = outputLayer_exp_masked / denom
  tau = outputLayer_masked / denom

  E_CCE = -tf.reduce_sum(tf.multiply(tau , tf.log(yy)), axis=1)
  def grad(dy):
      output_norm_e = tf.expand_dims(output_norm, -1)
      t1 = outputLayer / output_norm_e
      tau_t = tf.tile(tf.expand_dims(tau, -1), [1, 1, num_class*num_cluster_per_class])
      yy_t = tf.tile(tf.expand_dims(yy, -1), [1, 1, num_class*num_cluster_per_class])
      t2 = tau_t / output_norm_e
      eye_ = tf.tile(tf.expand_dims(tf.eye(num_class*num_cluster_per_class) , 0) ,  [batch_size , 1 , 1])
      t3 = tf.multiply(t2, yy_t - eye_ )
      t4 = tf.reduce_sum(t3, axis=1)
      t5 = tf.tile(tf.expand_dims(t4, -1), [1, 1, caps2_n_dims])
      tfinal = tf.multiply(t1, t5)
      return tfinal
  # return  (yy - tau)
  return E_CCE , grad
  # return E_CCE
...
caps2_output_sq = tf.squeeze(caps2_output,axis=[1,-1])

loss = ECC_loss(caps2_output_sq)
my_loss = tf.reduce_mean(loss, axis=0)

regularizer = tf.nn.l2_loss(W)
alpha_ = 0.01 #was 0.01
sigma_ = 0.1
